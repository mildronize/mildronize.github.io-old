...
import { sleep, getUuidfromPathname } from '../utils/utils';

const tocDepth = 3;
const enableToc = true;
const config = {
  metadata: {pathPrefix: ''}
}
...
const generateToCItems = (allMdx, location) => {
  const locationUuid = getUuidfromPathname(location.pathname);
  console.table(allMdx)
console.log(location)
  let finalNavItems = [];
  const isCurrentPage = (slug) =>
    slug === locationUuid || config.metadata.pathPrefix + slug === locationUuid;
  ...
  if (allMdx.edges !== undefined && allMdx.edges.length > 0) {
    allMdx.edges.every((item) => {
      ...
      let innerItems = [];
      if (item !== undefined) {
        if (isCurrentPage(item.node.fields.slug)) {
          console.log('is this page')
          const maxDepth = tocDepth;
...
const TableOfContents = ({ show, className, location }) => (
  <StaticQuery
    ...
    render={({ allMarkdownRemark }) => {
      const finalNavItems = generateToCItems(allMarkdownRemark, location);
